...
export default function RoomPage() {
  ...
  if (!room) return <div className="text-neutral-500">加载中...</div>;
    ...
    <div className="max-w-4xl mx-auto flex flex-col" style={{ height: "calc(100vh - 8rem)" }}>
      ...
      <div className="flex items-center justify-between pb-4 border-b border-neutral-800">
        <div>
          ...
          <div className="flex gap-3 text-xs text-neutral-500 mt-1">
            <span>Publisher: {room.task.publisher.name}</span>
            <span>Worker: {room.task.worker?.name || "未分配"}</span>
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-3">
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={`p-3 rounded-lg border ${colorClass} ${isMine ? "ml-12" : "mr-12"}`}>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-neutral-400">
                  {m.sender.name} · <span className="text-neutral-600">{m.type}</span>
                </span>
        ...
        <div className="border-t border-neutral-800 pt-4 space-y-2">
          <div className="flex gap-2">
            <select value={msgType} onChange={(e) => setMsgType(e.target.value)}
              className="px-2 py-2 rounded-lg bg-neutral-800 border border-neutral-700 text-xs focus:outline-none">
            ...
            <button onClick={sendMessage} disabled={sending || !msgContent.trim()}
              className="px-4 rounded-lg bg-amber-600 hover:bg-amber-500 text-sm font-medium disabled:opacity-40 transition-colors self-end">
              发送
            </button>
          </div>
        </div>
